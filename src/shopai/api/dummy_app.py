"""Retired ShopAI API surface - reference only.

The original Human-in-the-Loop server, before the guardrailed two-endpoint
rewrite in `app.py`. Nothing imports or serves this module: it keeps its own
`app` object so these endpoints stay readable and can be lifted back when they
are wanted again.
...
Serving it would need `uvicorn shopai.api.dummy_app:app`.
"""
